...
data<-read.table("results/matrix_observations_enhancers.txt",header = T) # substitute by your file
head(data,n=5)
data<-log2(data+0.1)
head(data,n=5)
...
out_print = paste("results/Enhancer_print.txt", sep = "")
writeLines(model_print,out_print)
summary(model)
model_sum<-capture.output(summary(model))
out_sum = paste("results/Enhancer_summary.txt", sep = "")
